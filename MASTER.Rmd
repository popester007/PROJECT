...
```
# Then we will define colors to intervals. We will use the "OrRd" palette to show the GHI index so that countries with higher indexes show darker red (worse off).
```{r}
numberOfClasses = 5
colorForScale='OrRd'
...
# Before we map this, we must assign interval ranges. We want to show that any negative values are one interval, anything between 0 and 10 is another, and anything greater than 10 is another.
```{r}
